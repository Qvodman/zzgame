...
function() {
			...
			initEvent: function() {
				...
				$(window).resize(function() {
					...
					if("a"==a) {
						...
						// $("#box_mage").css("background-color", color);
						e.nextLv.call(e);
					}
		...
		e = {
			...
			render: function(e, f) {
				...
				var h = Math.floor(Math.random() * 3 * 3),
					...
					j = this.getLvColor(i[1]);
				var biao = ["红","橙","黄","绿","青","蓝","紫","白","黑"];
				var color = ["#FF0000","#FF5809","#FFFF37","#82D900","#00FFFF","#0080FF","#FF00FF","#FFFFFF","#000000"];
				// a.find(b).eq(h).css("background-color",color);
				a.find(b).each(function(index){
					$(this).css("background-color", color[index]);
				});
				a.find(b).eq(h).data("type", "a");
				$("#box_mage").css("color", color[h+1]);
				$("#box_mage").text(biao[h]);
			},
			getColor: function(a) {
	...
	window.location.href="../score.jsp?lv="+ b;
} ;
